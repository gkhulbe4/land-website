...
function AllLands() {
  ...
  const deleteLand = (id) => {
    axios
      .delete(`http://localhost:3000/lands/${id}`, {
        headers: {
    ...
    <div>
      ...
      <div className="flex flex-col gap-6 pt-6 bg-gray-200">
        {landInfo.map((land) => (
          <div
            key={land._id}
            className="relative bg-white flex py-7 px-2 hover:opacity-80 hover:shadow-lg transition duration-200 ease-out border-t mx-5 rounded-lg"
          >
              ...
              <div className="flex justify-between items-end pt-5">
                ...
                <div>
                  <p className="text-lg font-semibold pb-2 lg:text-2xl">
                    ₹{new Intl.NumberFormat("en-IN").format(land.price)}
                  </p>
                </div>
              </div>
            </div>
